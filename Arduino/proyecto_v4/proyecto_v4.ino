...
float tempMax = 27;         //temp maxima, si sobrepasa prende ventilador y disminuye lamp
float tempMin = 20;         //temp minima, si baja aumenta lamp y apaga ventilador
int flagTemp = 0;         //bandera para verificar rango de temperatura
int counter = 0;
int counterMax = 100;      //cada cuanto
...
void loop(){
  ...
  if(tempActual > tempMax){ //prender ventilador y bajamos intensidad lampara
    writeFan(1);  //prende ventilador
    if(counter == counterMax){
      tempActual = readTemperature();
      Serial.println(String(tempActual)+F("|")+String(50));
      int lampValue = readLamp();
      writeLamp(String(lampValue-1));
      counter=0;
    }
    counter++;
  } else if(tempActual < tempMin){
    writeFan(0);  //apaga ventilador
    if(counter == counterMax){
      tempActual = readTemperature();
      Serial.println(String(tempActual)+F("|")+String(60));
      int lampValue = readLamp();
      writeLamp(String(lampValue+1));
      counter=0;
    }
	...
	if (Serial.available() > 0) {
		...
		switch(insPin){
          ...
          case '1':{ //escribir temperatura
            char insPin3 = instruction.charAt(2);
            writeTemp(instruction.charAt(2), instruction.substring(3,6));
            break;
          }
				}
				break;  
...
void writeTemp(char what, String value){
  switch(what){
    case '0':{ //minima
      tempMin = value.toFloat();
      break;
    }
    case '1':{ //maxima
      tempMax = value.toFloat();
      break;
    }
  }
}
